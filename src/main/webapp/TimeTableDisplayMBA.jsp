			...
			Collections.shuffle(valuess);
						...
						<table class="table table-bordered text-center">
							...
							<tbody>
								...
								%>		
<%!
int[][] matrix = new int[6][4]; // 6 days × 4 lectures
...
boolean tryGenerate() {
    ...
    // Randomly place 6 unique subjects (1–6) in the first column (column 0)
    List<Integer> firstCol = new ArrayList<>(Arrays.asList(1, 2, 3, 4, 5, 6));
    Collections.shuffle(firstCol);
    for (int i = 0; i < 6; i++) {
        temp[i][0] = firstCol.get(i);
    }

    // Prepare pool and remove already used (first column) values
    List<Integer> pool = new ArrayList<>(values);
    for (int i = 0; i < 6; i++) {
        pool.remove((Integer) temp[i][0]);
    }

    // Fill remaining columns (1–3)
    for (int i = 0; i < 6; i++) {
        for (int j = 1; j < 4; j++) {
            if (pool.isEmpty()) return false;
            int index = new Random().nextInt(pool.size());
            temp[i][j] = pool.remove(index);
        }
    }

    // Copy temp to main matrix
    for (int i = 0; i < 6; i++) {
        System.arraycopy(temp[i], 0, matrix[i], 0, 4);
    }
...
void generateMatrix() {
	System.out.println("<p style='color:green;'>Matrix generated!</p>");
    values.clear();
...
try {
    generateMatrix();
    out.println("<p style='color:green;'>Matrix generated successfully.</p>");
} catch (Exception ex) {
    out.println("<p style='color:red;'>Matrix generation error: " + ex.getMessage() + "</p>");
    ex.printStackTrace();
}
%>
...
// Initialize nameMap
for (int i = 1; i <= 6; i++) {
    nameMap.put(i, "SUB" + i); // default placeholder
}
						...
						<table class="table table-bordered text-center">
							<thead class="table-dark">
								<tr>
									...
									<% 
										if(sem.equals("1") || sem.equals("2")){
											out.println("<th>1:15 - 2:15 PM</th><th>2:15 - 3:15 PM</th>");
										}
									%>
								</tr>
							</thead>
...
for (int day = 0; day < 6; day++) {
    ...
    for (int slot = 0; slot < 4; slot++) {
        ...
        try {
            ...
            // Compare
            if (fac != null && fac.equals(new_fac)) flag = true;
